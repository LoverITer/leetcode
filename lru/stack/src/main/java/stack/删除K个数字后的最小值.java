...
public class 删除K个数字后的最小值 {
    ...
    /**
     * 贪心算法
     * <p>算法思路：一个很简单的道理需要明白，那就是在删除的时候应该尽可能的首先删除一个数字的高位，因为高位
     * 数字对数字的影响是最大的，因此我们可以想到：从左向右遍历原整数，当发现前一位数比后一位数大的时候就将前一位数删除
    ...
    /**
     * 贪心+桟 降低时间复杂度
     * <p>算法思路：上面贪心的思路不变，依然遍历数字字符串，在遍历的时候让每个数字都入栈，当栈顶栈顶数字比当前数字大的时候，
